...
Message* inputRequest(string &request) {
	...
	return new Message(ID, getCommadCode(COMMAND), static_cast<int>(REQUEST.length()), content);
}
...
void showResponse(Message *message) {
	...
	switch (responseCode) {
		...
		case LOGIN_FAIL_ANOTHER_DEVICE:
			printf("Login failed. Account already logged in on another device!\n");
			break;
